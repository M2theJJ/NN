            ...
            if self.enable_pruning is True:
                ...
                def update_mask(sparsity):
                    ...
                    pruning_threshold_index_uncapped = tf.dtypes.cast(
                        tf.math.round(self.num_weights_fp * (1 - sparsity)),
                        tf.int32)
...
class BFPBiasQuantizer(BFPQuantizer):
    ...
    def __call__(self, inputs, training, weights, **kwargs):
        ...
        if training is False:
            ...
            quantized_inputs = quantize_variable(variable=inputs, exp=weights["exp"], width=self.num_bits, clip=True,
...
class BFPQuantizeConfig(QuantizeConfig):
    def __init__(self, output_quantizer=None, weight_quantizer=None, bias_quantizer=None):
        ...
        if bias_quantizer is None:
            self.bias_quantizer = BFPBiasQuantizer(num_bits=8)
        else:
