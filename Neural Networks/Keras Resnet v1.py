...
def lr_schedule(epoch):
    ...
    # Get output & print it
    #model.layers[depth].output
    #print(model.layers[depth].output)
...
#    model_a = Model(inputs=model.input, outputs=model.get_layer(name=None, index=index).output)
...
logging.basicConfig(level=logging.DEBUG, filename='../Keras_Resnet_CIFAR10_v1.log')
...
# MJ:
# I know it's not very elegant, but I wanted to test out the different Resnet versions,
# I'll implement a final version including both versions and all the additions when they are done.
# From lines 360 on is where the issue lies.
# I'll keep on trying but upload this version to github the 17.6 13:46