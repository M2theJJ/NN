...
_STORE_SPARSITY = False


def stochastic_round(x):
    r, f = np.modf(x)
    return r + tf.random.binomial(1, r)
            ...
            if self.enable_pruning is True:
                with tf.name_scope("pruning"):
                    ...
                    def update_mask():
                        # TODO FIX FOR FC
                        if self.is_fc:
                            reordered_kernel = tf.transpose(abs_weights, [1, 0])
                        else:
                        ...
                        if self.is_fc:
                            mask = tf.transpose(reordered_mask, [1, 0])
                        else:
                    ...
                    weights["pruning_step"].assign(incremented_pruning_step)
...
#                                                           "running")
        ...
        if _STORE_SPARSITY:
            log.info("Storing sparsity of weights and activations")
        else:
            log.info("NOT Storing sparsity of weights and activations")
        ...
        def add_quantize_annotation(layer):
            ...
            for layer_type in quantization_map:
                ...
                if isinstance(layer, layer_type):  # and not (tf.keras.activations.get("softmax") == layer.activation):
                    ...
                    elif isinstance(pruning_policy, int):
                        layer_pruning = float(pruning_policy)
